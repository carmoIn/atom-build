  ...
  startNewBuild: function() {
    ...
    this.child.on('close', function(exitCode) {
      ...
      if (0 === exitCode) {
        ...
        // TODO: Hack to execute a command when compile finish
        for (var i in this.cmd.finish) {
            if(this.cmd.finish[i][0])
            {
                var args = this.cmd.finish[i][2] || [];
                args = _.map(args, this.replace);
                var path = this.cmd.finish[i][1] || atom.project.getPaths()[0];

                this.child = child_process.spawn(
                    this.cmd.finish[i][0], args, { cwd: this.replace(path) }
                );
            }
        }
      }
